...
std::expected<std::unique_ptr<CustomIFS>, std::string> CustomIFS::create(
    const VulkanContext& context,
    vk::Device device
) {
    auto backend = std::unique_ptr<CustomIFS>(new CustomIFS(context, device));

    if (auto result = backend->initialize(); !result) {
        return std::unexpected(result.error());
    }

    Logger::instance().info("Created CustomIFS backend");
    return backend;
}
...
CustomIFS::CustomIFS(CustomIFS&& other) noexcept
...
CustomIFS& CustomIFS::operator=(CustomIFS&& other) noexcept {
    if (this != &other) {
        cleanup();
...
std::expected<void, std::string> CustomIFS::initialize() {
    // Load compute shader
    auto shader_result = Shader::create_shader(
        m_device,
        "ifs_modular/backends/custom_ifs",
        "main"
    );
    if (!shader_result) {
...
std::expected<void, std::string> CustomIFS::create_descriptor_layout() {
    if (!m_compute_shader) {
...
std::expected<void, std::string> CustomIFS::create_pipeline() {
...
void CustomIFS::wait_compute_complete() {
    if (m_compute_fence) {
        [[maybe_unused]] auto result = m_device.waitForFences(m_compute_fence, true, UINT64_MAX);
    }
}
...
void CustomIFS::reallocate_particle_buffer(uint32_t new_count) {
