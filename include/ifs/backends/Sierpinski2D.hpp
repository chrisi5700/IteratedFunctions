 ...
 */
class Sierpinski2D : public IFSBackend {
public:
    /**
     * @brief Create Sierpinski2D backend
     *
     * @param context Vulkan context
     * @param device Vulkan device
     * @return Sierpinski2D instance or error message
     */
    static std::expected<std::unique_ptr<Sierpinski2D>, std::string> create(
        const VulkanContext& context,
        vk::Device device
    );

    ~Sierpinski2D() override;

    // Non-copyable, movable
    Sierpinski2D(const Sierpinski2D&) = delete;
    Sierpinski2D& operator=(const Sierpinski2D&) = delete;
    Sierpinski2D(Sierpinski2D&&) noexcept;
    Sierpinski2D& operator=(Sierpinski2D&&) noexcept;

    // IFSBackend interface
    [[nodiscard]] std::string_view name() const override { return "Sierpinski Triangle (2D)"; }
